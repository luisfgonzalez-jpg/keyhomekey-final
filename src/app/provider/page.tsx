...
export default function ProviderDashboard() {
  ...
  async function loadProviderData() {
    try {
      ...
      // 4. Get ONLY tickets assigned to THIS provider
      const { data: ticketsData, error: ticketsError } = await supabase
        ...
        `)
        .eq('assigned_provider_id', provider.id) // ← CRITICAL: Only THIS provider's tickets
      ...
      // 5. Calculate stats from ONLY assigned tickets
  ...
  async function handleRejectTicket(ticketId: string) {
    ...
    try {
      ...
      alert('Error al rechazar ticket: ' + error.message);
    }
  }
  ...
  async function handleCompleteTicket(ticketId: string) {
    ...
    try {
      ...
      alert('Error al completar ticket: ' + error.message);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        {/* Stats Cards - ONLY from assigned tickets */}
        ...
        {/* Tickets List - ONLY assigned tickets */}
                ...
                {tickets.map((ticket) => (
                  ...
                  >
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-200 text-xs text-slate-500">
                      Creado el {new Date(ticket.created_at).toLocaleDateString('es-CO', { 
                        year: 'numeric', 
                        month: 'long', 
                        day: 'numeric' 
                      })}
